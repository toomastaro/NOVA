...
class InlineNovaStat(InlineKeyboardBuilder):
    ...
    def main_menu(cls):
        ...
        # kb.button(text="⬅️ Назад", callback_data="NovaStat|exit") # Удалено по запросу
        kb.adjust(1)
        return kb.as_markup()
    ...
    def collection_view(cls, collection: Collection, channels: List[CollectionChannel]):
        ...
        kb.button(
            ...
            callback_data=f"NovaStat|col_analyze|{collection.id}",
        )
        kb.button(
            text="Добавить канал",
            callback_data=f"NovaStat|col_add_channel|{collection.id}",
        )
        kb.button(
            text="Удалить канал",
            callback_data=f"NovaStat|col_del_channel_list|{collection.id}",
        )
        kb.button(
            text="Переименовать", callback_data=f"NovaStat|col_rename|{collection.id}"
        )
        kb.button(
            text="Удалить коллекцию",
            callback_data=f"NovaStat|col_delete|{collection.id}",
        )
        kb.button(text="⬅️ Назад", callback_data="NovaStat|collections")
        kb.adjust(1)
        return kb.as_markup()

    @classmethod
    def collection_channels_delete(
        cls, collection_id: int, channels: List[CollectionChannel]
    ):
        kb = cls()
        for ch in channels:
            kb.button(
                text=f"❌ {ch.channel_identifier}",
                callback_data=f"NovaStat|col_del_channel|{collection_id}|{ch.id}",
            )
        kb.button(text="⬅️ Назад", callback_data=f"NovaStat|col_open|{collection_id}")
        kb.adjust(1)
        return kb.as_markup()
    ...
    def analysis_result(cls):
        kb = cls()
        kb.button(
            text="Рассчитать цену рекламы", callback_data="NovaStat|calc_cpm_start"
        )
        kb.button(text="⬅️ Назад", callback_data="NovaStat|main")
        kb.adjust(1)
        return kb.as_markup()
