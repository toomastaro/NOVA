...
async def grant_subscription(
    ...
    logger.info(
        f"Выдача подписки пользователю {user_id} на {len(chosen)} объектов типа {object_type} на {total_days} дней"
    )
    ...
    for obj_id in chosen:
        try:
            if object_type == "channels":
                ...
                logger.debug(
                    f"Подписка для канала {channel.title} ({channel.id}) обновлена до {subscribe_value}"
                )
                ...
                logger.debug(
                    f"Подписка для бота {user_bot.title} ({user_bot.id}) обновлена до {subscribe_value}"
                )

        except Exception as e:
            logger.error(
                f"Ошибка при выдаче подписки для объекта {obj_id} (тип {object_type}): {e}",
                exc_info=True,
            )
