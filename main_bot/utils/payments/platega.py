...
class Platega:
    def __init__(
        ...
        self.client = AsyncClient(base_url=self.base_url, headers=self.headers, timeout=10.0)
...
platega_api = Platega(
    merchant_id=Config.PLATEGA_MERCHANT, secret_key=Config.PLATEGA_SECRET
)


async def main():
    r = await platega_api.create_invoice(
        order_id=str(uuid.uuid4()), amount=10, description="123"
    )
    print(r)
