...
async def run_analysis_logic(message: types.Message, channels: list, depth: int, state: FSMContext, status_msg: types.Message = None):
    ...
    await message.answer(report, reply_markup=keyboards.analysis_result(), parse_mode="HTML", link_preview_options=types.LinkPreviewOptions(is_disabled=True))
...
async def calculate_and_show_price(message: types.Message, cpm: int, state: FSMContext, is_edit: bool = False):
    ...
    if is_edit:
        await message.edit_text(report, reply_markup=keyboards.cpm_result(), parse_mode="HTML", link_preview_options=types.LinkPreviewOptions(is_disabled=True))
    else:
        await message.answer(report, reply_markup=keyboards.cpm_result(), parse_mode="HTML", link_preview_options=types.LinkPreviewOptions(is_disabled=True))
