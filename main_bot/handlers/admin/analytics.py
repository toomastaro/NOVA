...
async def show_analytics_summary(call: types.CallbackQuery) -> None:
    ...
    conv_sub = (users_with_sub / users_with_channels * 100) if users_with_channels else 0
...
def get_router() -> Router:
    router.callback_query.register(
        show_analytics_menu, F.data == "AdminAnalytics|menu"
    )
    router.callback_query.register(
    ...
    router.callback_query.register(
        show_analytics_top, F.data == "AdminAnalytics|top"
    )
    return router
