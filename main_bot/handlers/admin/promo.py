...
async def get_promo(message: types.Message, state: FSMContext):
    ...
    if len(temp) < 4:
        return await message.answer("Что-то забыл указать")
    ...
    await db.add_promo(name=name, amount=amount, use_count=count_use, discount=discount)

    await state.clear()
    await message.answer("Промокод был успешно добавлен")
