...
async def get_value(message: types.Message, state: FSMContext):
    ...
    post = await db.update_story(post_id=post.id, return_obj=True, **kwargs)
...
async def accept(call: types.CallbackQuery, state: FSMContext):
    ...
    await db.update_story(post_id=post.id, **kwargs)
...
def hand_add():
    ...
    router.callback_query.register(
        cancel_message, F.data.split("|")[0] == "InputStoriesCancel"
    )
    ...
    # Choice
    router.callback_query.register(
        choice_channels, F.data.split("|")[0] == "ChoiceStoriesChannels"
    )
    # Finish Params
    router.callback_query.register(
        finish_params, F.data.split("|")[0] == "FinishStoriesParams"
    )
    router.callback_query.register(
        choice_delete_time, F.data.split("|")[0] == "GetDeleteTimeStories"
    )
    router.callback_query.register(
        cancel_send_time, F.data.split("|")[0] == "BackSendTimeStories"
    )
