...
async def choice(call: types.CallbackQuery):
    ...
    await call.message.edit_text(
        ...
        reply_markup=keyboards.manage_channel(data="ManageChannelStories"),
    )


async def cancel(call: types.CallbackQuery):
    channels = await db.get_user_channels(user_id=call.from_user.id, sort_by="stories")
    return await call.message.edit_text(
...
def hand_add():
    ...
    router.callback_query.register(
        cancel, F.data.split("|")[0] == "BackAddChannelStories"
    )
    router.callback_query.register(
