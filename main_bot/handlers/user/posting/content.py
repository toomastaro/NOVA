...
async def choice_channel(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        ...
        reply_markup=keyboards.choice_row_content(posts=posts, day=day),
    )
...
async def choice_row_content(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer(
        text("post:content").format(*send_date_values, channel.emoji_id, channel.title),
...
def hand_add():
    ...
    router.callback_query.register(
        choice_row_content, F.data.split("|")[0] == "ContentPost"
    )
    router.callback_query.register(
