...
async def show_manage_hello_message(message: types.Message,  state: FSMContext):
    ...
    await message.answer(
        text("manage_hello_message").format(
            data.get("idx")
        ),
        reply_markup=keyboards.manage_hello_message(
...
async def manage_hello_message(call: types.CallbackQuery, state: FSMContext):
    ...
    if temp[1] == "change":
        ...
        post_id = await answer_message(call.message, MessageOptionsHello(**hello_message.message))
        await state.update_data(
...
def get_router():
    ...
    router.callback_query.register(manage_hello_message, F.data.split("|")[0] == "ManageHelloMessage")
    router.callback_query.register(choice_hello_message_delay, F.data.split("|")[0] == "ChoiceHelloMessageDelay")
    router.callback_query.register(manage_hello_message_post, F.data.split("|")[0] == "ManagePostHelloMessage")
