...
from main_bot.utils.schemas import ByeAnswer, HelloAnswer
...
async def show_channel_setting(
    ...
    channel = await db.get_channel_by_chat_id(chat_id=data.get("chat_id"))
...
async def choice_channel(
    ...
    if temp[1] in ["next", "back"]:
        ...
        channels = [
            await db.get_channel_by_chat_id(chat.id) for chat in channel_ids_in_bot
        ]
...
async def choice(call: types.CallbackQuery, state: FSMContext, db_obj: Database):
    ...
    channel_setting = await db.get_channel_bot_setting(chat_id=data.get("chat_id"))
...
async def show_captcha(
    ...
    channel_captcha_list = await db.get_all_captcha(chat_id=setting.id)
...
async def show_hello(message: types.Message, setting: ChannelBotSetting):
    hello_messages = await db.get_hello_messages(chat_id=setting.id)
