...
async def show_manage_captcha(message: types.Message, state: FSMContext):
    ...
    captcha = await db.get_captcha(message_id=data.get("captcha_id"))
...
async def choice(
    ...
    if temp[1] in ["next", "back"]:
        channel_captcha_list = await db.get_all_captcha(chat_id=channel_settings.id)
...
async def manage_hello_message(
    ...
    if temp[1] in ["cancel", "delete"]:
        if temp[1] == "delete":
            await db.delete_captcha(data.get("captcha_id"))
...
async def get_buttons(message: types.Message, state: FSMContext):
    data = await state.get_data()
    hello_obj = await db.get_captcha(message_id=data.get("captcha_id"))
    ...
    await db.update_captcha(captcha_id=hello_obj.id, message=hello_message.model_dump())
