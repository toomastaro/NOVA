...
from main_bot.utils.schemas import Media, MessageOptions
...
async def choice_bots(call: types.CallbackQuery, state: FSMContext):
    ...
    objects = await db.get_user_channels(call.from_user.id, from_array=channel_ids)
    ...
    folders = await db.get_folders(
        user_id=call.from_user.id, folder_type=FolderType.BOT
    )
    ...
    if temp[1] == "choice_all":
        if len(chosen) == len(objects) and len(chosen_folders) == len(folders):
            ...
            extend_list = [i.id for i in objects if i.id not in chosen and i.subscribe]
            if not extend_list:
                return await call.answer(text("error_sub_all:bots"))
            ...
            if folders:
                for folder in folders:
                    sub_channels = []
                    for chat_id in folder.content:
                        user_bot = await db.get_bot_by_id(int(chat_id))
    ...
    if temp[1].replace("-", "").isdigit():
        ...
        if temp[3] == "bot":
            if resource_id in chosen:
                ...
                user_bot = await db.get_bot_by_id(resource_id)
                if not user_bot.subscribe:
                    return await call.answer(text("error_sub_channel:bots"))
...
async def manage_post(call: types.CallbackQuery, state: FSMContext):
    ...
    if temp[1] == "next":
        ...
        await call.message.delete()
        return await call.message.answer(
            text("choice_bots:post").format(0, ""),
            reply_markup=keyboards.choice_objects(
                resources=objects,
                chosen=[],
                folders=folders,
...
async def get_value(message: types.Message, state: FSMContext):
    ...
    post = await db.update_bot_post(post_id=post.id, return_obj=True, **kwargs)
...
async def finish_params(call: types.CallbackQuery, state: FSMContext):
    ...
    if temp[1] == "send_time":
        await call.message.edit_text(
            ...
            reply_markup=keyboards.back(data="BackSendTimeBotPost"),
        )
...
async def get_send_time(message: types.Message, state: FSMContext):
    ...
    if is_edit:
        ...
        if post.send_time is not None:
            ...
            send_date = datetime.fromtimestamp(post.start_timestamp or int(time.time()))
        send_date_values = (
            send_date.day,
...
async def accept(call: types.CallbackQuery, state: FSMContext):
    ...
    await db.update_bot_post(post_id=post.id, **kwargs)
...
def hand_add():
    ...
    # Values
    router.callback_query.register(
        cancel_value, F.data.split("|")[0] == "ParamBotPostCancel"
    )
    router.message.register(
    ...
    router.callback_query.register(
        cancel_send_time, F.data.split("|")[0] == "BackSendTimeBotPost"
    )
