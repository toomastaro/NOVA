...
class UserFolderCrud(DatabaseMixin):
    ...
    async def update_folder(self, folder_id: int, **kwargs):
        await self.execute(
            update(UserFolder).where(
                UserFolder.id == folder_id
            ).values(**kwargs)
        )

    async def get_folders(self, user_id: int, folder_type: FolderType = FolderType.CHANNEL):
