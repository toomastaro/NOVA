...
from main_bot.database.novastat.model import NovaStatSettings, Collection, CollectionChannel
...
class NovaStatCrud(DatabaseMixin):
    ...
    async def update_novastat_settings(self, user_id: int, **kwargs):
        await self.execute(
            update(NovaStatSettings).where(NovaStatSettings.user_id == user_id).values(**kwargs)
        )
    ...
    async def get_collection_channels(self, collection_id: int):
        return await self.fetch(
            select(CollectionChannel).where(CollectionChannel.collection_id == collection_id)
        )

    async def create_collection(self, user_id: int, name: str):
        await self.execute(
            insert(Collection).values(user_id=user_id, name=name)
        )
    ...
    async def delete_collection(self, collection_id: int):
        ...
        await self.execute(
            delete(CollectionChannel).where(CollectionChannel.collection_id == collection_id)
        )
        await self.execute(
    ...
    async def rename_collection(self, collection_id: int, new_name: str):
        await self.execute(
            update(Collection).where(Collection.id == collection_id).values(name=new_name)
        )

    async def add_channel_to_collection(self, collection_id: int, channel_identifier: str):
        await self.execute(
            insert(CollectionChannel).values(collection_id=collection_id, channel_identifier=channel_identifier)
        )
