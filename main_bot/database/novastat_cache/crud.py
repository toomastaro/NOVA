...
class NovaStatCacheCrud:
    ...
    async def get_cache(self, channel_identifier: str, horizon: int) -> Optional[NovaStatCache]:
    ...
    async def is_cache_fresh(self, channel_identifier: str, horizon: int, max_age_seconds: int = 3600) -> bool:
    ...
    async def set_cache(
        ...
        if cache:
            ...
            stmt = update(NovaStatCache).where(
                NovaStatCache.id == cache.id
            ).values(
    ...
    async def clear_stale_refresh_flags(self, max_age_seconds: int = 600) -> None:
        ...
        stmt = update(NovaStatCache).where(
            NovaStatCache.refresh_in_progress == True,
